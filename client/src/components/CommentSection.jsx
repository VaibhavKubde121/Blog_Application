  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCommentError(data.message || "Error submitting comment");
      }
    } catch (error) {
  ...
  useEffect(() => {
    const getComments = async () => {
      try {
        ...
        setCommentError(error.message || "Error fetching comments");
      }
    };
    getComments();
  }, [postId]);
    ...
    <div className="max-w-2xl mx-auto w-full p-3">
      {currentUser ? (
        ...
        <div className="text-sm text-teal-500 my-5 flex gap-2">
          ...
          <Link className="text-red-500 hover:underline" to={"/signin"}>
            {" "}
            Sign in
          </Link>
        </div>
